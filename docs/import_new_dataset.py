...
json_filename = 'dataset_linkedin-profile-search_2026-08-28_00-09-20-952.json'
...
$jsonFile = __DIR__ . '/new_dataset.json';
...
foreach ($profiles as $p) {
    ...
    // Deal estimation
    $amount = 6500.00;
    if (preg_match('/trading|hedge|capital|director|founder|vp|head|ceo|cto|cfo/i', $companyName . ' ' . $headline)) {
        $amount = 12500.00;
    } elseif (preg_match('/intern|assistant|analyst|coordinator/i', $jobPos)) {
        $amount = 3500.00;
    }
...
with open('docs/import_new_dataset.php', 'w', encoding='utf-8') as out_f:
    out_f.write(php_script)

print("Generated docs/import_new_dataset.php successfully.")
