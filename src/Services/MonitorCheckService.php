...
class MonitorCheckService
{
    public function checkUptime(Monitor $monitor): array
    {
        ...
        // Pin DNS resolution so cURL cannot re-resolve to a different (internal)
        // address than the guard validated — defends against DNS rebinding.
        $resolveOptions = [];
        if (! $allowPrivate) {
            ...
            if ($host) {
                $pinned = MonitorUrlGuard::resolvePublicIps($host);

                if ($pinned === []) {
                    $result['error'] = 'URL host resolves to a non-routable address.';

                    return $result;
                }

                $resolveOptions[CURLOPT_RESOLVE] = [
                    $host.':'.$port.':'.implode(',', $pinned),
                ];
            }
        }
        ...
        try {
            $response = Http::timeout($timeout)
                ->withOptions(array_merge([
                    // Disable redirect-following so a monitored host can't bounce
                    // us into an internal URL the guard never saw.
                    'allow_redirects' => false,
                    'curl' => $resolveOptions,
                    'on_stats' => function (TransferStats $stats) use (&$transferSeconds) {
    ...
    public function checkSsl(Monitor $monitor): array
    {
        ...
        // Connect to a vetted IP rather than re-resolving via the system
        // resolver — defends against DNS rebinding between the guard check and
        // the socket open. The peer_name SNI value still uses the hostname so
        ...
        if (! $allowPrivate) {
            ...
            $connectTarget = $pinned[0];
            if (str_contains($connectTarget, ':')) {
